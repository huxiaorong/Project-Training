...
public class RecordDaoImpl extends BaseDao<Record, Integer> {
	...
	public List<Record> findThisWeek(int userId){
		...
		try {
			return gson.fromJson(
					gson.toJson(super.findBySql("select * from record where YEARWEEK(date_format(studiedTime,'%Y-%m-%d')) = YEARWEEK(now()) and userId=?", 
							new Object[] {userId})),
					new TypeToken<List<Record>>() {
			        }.getType());
		} catch (Exception e) {
	...
	public List<Record> findThisMonth(int userId){
		...
		try {
			return gson.fromJson(
					gson.toJson(super.findBySql("select * from record where date_format(studiedTime,'%Y-%m')=date_format(now(),'%Y-%m') and userId=?", 
							new Object[] {userId})),
					new TypeToken<List<Record>>() {
			        }.getType());
		} catch (Exception e) {
